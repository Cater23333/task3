...
class DiffPIR(nn.Module):
    def __init__(self,noise_level,model_diffusion, vec_t=None, model_out_type='pred_xstart', \
                 diffusion=None, ddim_sample=False, alphas_cumprod=None, **model_kwargs):
    ...
    def forward(self, x):
        ...
        out=model_fn(x, self.noise_level,model_diffusion, self.vec_t, self.model_out_type, \
                 self.diffusion,self.ddim_sample, self.alphas_cumprod, **self.model_kwargs)
        #out=out.reshape(-1)
        return out
